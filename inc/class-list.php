...
/**************************************************************
 *
 * table class for Hair list admin page 
 *
 **************************************************************/
...
if(!class_exists('H_admin_list')){
	// CREATE A PACKAGE CLASS
	class H_admin_list extends WP_List_Table{
		...
		function __construct($table_rows = array(), $total = 0){
			...
			parent::__construct(array(
				'singular'  => 'hair',		//singular name of the listed records
				'plural'	=> 'hairs',		//plural name of the listed records
            ...
            global $H_conf;
			$this->rows_per_page = $H_conf['rows_per_page'];
		}
		 ...
		 */
		function column_hair_name($item){
			
			//Build row actions
			$actions = array(
				'edit'		=> sprintf('<a href = "?page=%s&action=%s&hair=%s">'.__('Edit', 'hairstyle').'</a>', $_REQUEST['page'], 'edit', $item->ID),
				'delete'	=> sprintf('<a href = "#" onclick = "javascript: removeHair(\''.__('Do you remove this documentation really?', 'hairstyle').'\', \'%s\');">'.__('Remove', 'hairstyle').'</a>', $item->ID),
			);
		...
		function get_columns(){
			$columns = array(
				'hair_name'		=> __('Hair Name', 'hairstyle'),
				'author'		=> __('Author', 'hairstyle'),
				'hair_shortcode' => __('Shortcode', 'hairstyle'),
				'date'			=> __('Date', 'hairstyle')
			);
			return $columns;
		}
		...
		function get_sortable_columns(){
			$sortable_columns = array(
				/*'hair_name'	 => array('hair_name', false),	 //true means it's already sorted
		...
		function prepare_items(){
			...
			function usort_reorder($a, $b){
				...
				$orderby =(!empty($_REQUEST['orderby']))? $_REQUEST['orderby']: 'doc_name'; //If no sort, default to title
